...
import dev.fastcampus.webfluxcoroutine.config.Locker
...
class ArticleService(
    ...
    suspend fun getAllCached(request: QryArticle): Flow<Article> {
        ...
        return cache.get(key) {
            getAll(request).toList() // flow를 list Article 변경하고 다시 flow로 변경
        }?.asFlow() ?: emptyFlow()
    }
    ...
    suspend fun addBalance(id: Long, amount: Int): Article {
        ...
        return locker.lock(key){
            val account = repository.findById(id) ?: throw NoArticleFound("id: $id")
            delay(3.seconds)
